...
# Paths
WEBARENA_VERIFIED_DATASET = Path("/home/demiw/webarena-verified/assets/dataset/webarena-verified.json")
...
class VerifiedWebArenaTask(AbstractBrowserTask):
    ...
    def setup(self, page: playwright.sync_api.Page) -> tuple[str, dict]:
        ...
        self.agent_response_file = Path(tempfile.gettempdir()) / f"agent_response_{self.task_id}.json"
...
"""
    ...
    def validate(
        ...
        if not agent_response and self.agent_response_file and self.agent_response_file.exists():
            try:
        ...
        try:
            score = self._run_verified_evaluation(agent_response)
    ...
    def _extract_response_from_action(self, action_text: str) -> dict | None:
        ...
        import re
        match = re.search(r'save_agent_response\s*\(\s*(\{[\s\S]*?\})\s*\)', action_text)
        if match:
            try:
    ...
    def _run_verified_evaluation(self, agent_response: dict, har_path: Path | None = None) -> float:
        ...
        try:
            ...
            config = WEBARENA_VERIFIED_CONFIG if WEBARENA_VERIFIED_CONFIG.exists() else None
            ...
            if har_path and har_path.exists():
                ...
                logger.info(f"Using HAR file for evaluation: {har_path} ({har_path.stat().st_size:,} bytes)")
            else:
            ...
            try:
                ...
                logger.info(f"Evaluation result: score={result.score}, status={result.status}")
                if result.evaluators_results:
                    for er in result.evaluators_results:
                        logger.info(f"  - {er.evaluator_name}: {er.status} (score: {er.score})")
            ...
            logger.warning(f"webarena_verified not available ({e}), using basic validation")
    ...
    for task in all_tasks:
        ...
        if env_id not in gym.envs.registry:
            gym.register(
                ...
                kwargs={"task_entrypoint": VerifiedWebArenaTask, "task_kwargs": {"task_id": task_id}},
            )
