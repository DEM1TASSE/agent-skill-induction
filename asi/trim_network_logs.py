...
def _convert_params_to_text(entries: list[dict[str, Any]]) -> int:
    """Convert postData.params array to text field for evaluator compatibility.

    Playwright's HAR format uses params array for form-encoded data, but
    the evaluator's parse_har_content expects text field. This converts
    params to urlencoded text format.

    Modifies entries in-place to add text field from params.

    Args:
        entries: List of HAR entry dicts

    Returns:
        Number of postData entries converted
    """
    ...
    for entry in entries:
        ...
        # Check if we have params but empty text (Playwright format)
        params = post_data.get("params")
        text = post_data.get("text", "")

        if params and not text:
            # Convert params array to urlencoded text
            # params format: [{"name": "key", "value": "val"}, ...]
            params_dict = {p["name"]: p["value"] for p in params}
            post_data["text"] = urlencode(params_dict)
            converted_count += 1
...
def trim_har_file(input_path: Path, output_path: Path) -> dict[str, Any]:
    ...
    After filtering, performs the following transformations:
    1. Sanitizes sensitive authorization headers from remaining entries:
    ...
    2. Converts postData.params arrays to text field (Playwright HAR compatibility)
    ...
    Returns:
        ...
        - request_headers_sanitized: Number of request headers sanitized
        - response_headers_sanitized: Number of response headers sanitized
        - postdata_converted: Number of POST data entries converted from params to text
    ...
    # Convert postData.params to text for evaluator compatibility
    postdata_converted = _convert_params_to_text(trimmed_entries)
    ...
    return {
        ...
        "postdata_converted": postdata_converted,
